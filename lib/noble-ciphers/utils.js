...
/** 微信小程序等环境可能无 TextEncoder/TextDecoder，使用纯 JS 回退 */
function utf8ToBytesFallback(str) {
    const bytes = [];
...
function bytesToUtf8Fallback(bytes) {
    ...
    while (i < len) {
        ...
        if (c < 0x80) {
            ...
            if (codePoint > 0xffff) {
                const c1 = codePoint - 0x10000;
                out += String.fromCharCode(0xd800 + (c1 >> 10), 0xdc00 + (c1 & 0x3ff));
            }
            else {
...
function utf8ToBytes(str) {
    ...
    return utf8ToBytesFallback(str);
}
/**
...
function bytesToUtf8(bytes) {
    ...
    return bytesToUtf8Fallback(bytes);
}
/**
