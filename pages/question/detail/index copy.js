...
Page({
    ...
    this.setData({
      ...
      setTimeout(() => {
        this.setData({
          ...
          console.log('引导显示状态:', this.data.showGuide);
          
          // 强制检查并可能重新渲染
          this.forceGuideDisplay();
        });
      }, 100);
    });
  },
  ...
  forceGuideDisplay() {
    // 方法1: 通过改变数据触发重新渲染
    const currentSteps = this.data.guideSteps;
    this.setData({
      guideSteps: [],
      showGuide: false
    }, () => {
  ...
  checkPageLayers() {
    ...
    // 检查 t-guide
    const guideQuery = wx.createSelectorQuery();
    guideQuery.select('t-guide').boundingClientRect();
    guideQuery.exec((res) => {
      console.log('t-guide 信息:', res[0]);
    });
    
    // 检查可能的遮挡元素
    const elements = ['t-action-sheet', '.floating-bottom-bar', 't-popup'];
    elements.forEach(selector => {
      const query = wx.createSelectorQuery();
      query.select(selector).boundingClientRect();
      query.exec((res) => {
        if (res[0]) {
          console.log(`${selector} 存在，可能遮挡`, res[0]);
        }
      });
    });
  },
